...
pub fn sha256_demo() {
    ...
    let params = profile("generating sha256 parameters...", || {
    ...
    let proof = profile("creating sha256 proof...", || {
    ...
    let verify_result = profile("verifying sha256 proof...", || {
...
fn profile<T, F: FnOnce() -> T>(description: &str, func: F) -> T {
    ...
    println!("Time elapsed in {} is: {:?}", description, duration);
